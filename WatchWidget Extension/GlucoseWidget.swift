...
struct GlucoseEntry: TimelineEntry {
    ...
    static var placeholder: GlucoseEntry {
        GlucoseEntry(date: Date(), glucose: "120", trend: "->", age: "1m", isStale: false, dateString: "Apr 2")
    }
}
...
struct GlucoseTimelineProvider: TimelineProvider {
    ...
    private func readGlucose() -> GlucoseEntry {
        ...
        // Read from shared app group UserDefaults (written by watch extension)
        ...
        guard let glucoseValue = defaults.double(forKey: "widgetGlucoseValue") as Double?,
              glucoseValue > 0,
              let glucoseDate = defaults.object(forKey: "widgetGlucoseDate") as? Date else {
            return GlucoseEntry(date: Date(), glucose: "---", trend: "", age: "", isStale: true, dateString: dateStr)
        }

        let trendSymbol = defaults.string(forKey: "widgetGlucoseTrend") ?? ""
        let unit = defaults.string(forKey: "widgetGlucoseUnit") ?? "mg/dL"

        let glucoseStr: String
        if unit == "mmol/L" {
            glucoseStr = String(format: "%.1f", glucoseValue / 18.0)
        } else {
...
struct InlineView: View {
    let entry: GlucoseEntry
    var body: some View { Text("\(entry.glucose)\(entry.trend) | \(entry.dateString) | \(entry.age)") }
}
